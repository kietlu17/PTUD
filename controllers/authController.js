...
async function login(req, res) {
    ...
    try {
        ...
        req.session.save(async(err) => {
            ...
            // 3. Logic cho GIÁO VIÊN 👈 PHẦN THAY ĐỔI
            if (user.role.TenVaiTro === 'giáo viên') {
                // Tìm thông tin giáo viên dựa trên username (giả định username là MaGV)
                const giaoVien = await GiaoVien.findOne({
                    where: { MaGV: username },
                    include: [
                        { model: Truong, as: 'truong' },
                        { model: Lop, as: 'lopChuNhiem' } // nếu có quan hệ này trong model
                    ],
                });

                if (!giaoVien) {
                    return res.status(404).json({ error: 'Không tìm thấy thông tin giáo viên hoặc lớp chủ nhiệm liên quan' });
                }

                // Lưu id_GiaoVien vào session
                req.session.user.giaoVienId = giaoVien.id;

                // Trả về dashboard giáo viên với thông tin chi tiết
                return res.status(200).render('dashboard-giaovien', {
                    giaoVien: {
                        ...giaoVien.toJSON(),
                        Truong: giaoVien.truong ? .name || 'Chưa cập nhật',
                        LopChuNhiem: giaoVien.lopChuNhiem ? .TenLop || 'Không chủ nhiệm lớp nào',
                    }
                });
            }
