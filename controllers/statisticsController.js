...
exports.getStatisticsData = async (req, res) => {
    try {
        ...
        const whereClause = { NamHoc: namHoc, HocKy: hocKy };
        ...
        // Tính toán thống kê
        let stats = { total: listDiem.length, gioi: 0, kha: 0, tb: 0, yeu: 0, dat: 0, khongDat: 0 };
        
        const dataTable = listDiem.map(d => {
            const dtb = parseFloat(d.DiemTrungBinh || 0);
            
            // Logic xếp loại (Ví dụ đơn giản)
            if (dtb >= 8.0) stats.gioi++;
            else if (dtb >= 6.5) stats.kha++;
            else if (dtb >= 5.0) stats.tb++;
            else stats.yeu++;

            if (dtb >= 5.0) stats.dat++;
            else stats.khongDat++;
            ...
            return {
                ...
                mon: d.monHoc.TenMon,
                dtb: dtb.toFixed(1),
                // Có thể thêm các cột điểm thành phần nếu cần
            };
        });

        res.json({ success: true, stats, data: dataTable });
